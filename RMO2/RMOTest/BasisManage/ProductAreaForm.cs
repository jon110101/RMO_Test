...
{
    public partial class ProductAreaForm : UserControl
    {
        public ProductAreaForm()
        {
            InitializeComponent();
        }

         private SubForm _ownerFrom;
         public ProductAreaForm(SubForm ownerForm)
        {
            InitializeComponent();
         ...
         /// 删除一条数据
         /// </summary>
         public bool Delete(string Company_Id, string ProductArea_Id)
         {
             StringBuilder strSql = new StringBuilder();
             strSql.Append("delete from ProductArea ");
             ...
             parameters[1].Value = ProductArea_Id;
         ...
         {
             StringBuilder strSql = new StringBuilder();
             strSql.Append("select  ROW_NUMBER() over (order by getdate()) as  ITM,Company_Id,ProductArea_Id,Desc_01,Parent_ProductArea_Id,Remark,Usr__Id,Create__Date from ProductArea  ");
         ...
         private void BtnQuery_Click(object sender, EventArgs e)
         {
             ...
             {
                 _NotNullWhere.Append(string.Format("and ProductArea_Id between '{0}'  and '{1}'", this.textBoxContainButton1.ID, this.textBoxContainButton2.ID));
             }
         ...
         {
             ProductAreaManage _Manage = new ProductAreaManage();
             _Manage.Edit = "ADD";
             ...
             if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
             {
                 if (dataGridView1.CurrentRow != null)
                 {
                     ...
                     if (_dr != null)
                     {
                         ...
                         _ProductArea_Id = string.IsNullOrEmpty((_dr.Cells["ProductArea_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["ProductArea_Id"].Value.ToString()).ToString();
                     }
                     ProductAreaManage _Manage = new ProductAreaManage();
                     _Manage.Edit = "UPD";
                     _Manage.ProductArea_Id = _ProductArea_Id;
             ...
             if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
             {
                 if (dataGridView1.CurrentRow != null)
                 {
                     ...
                     if (_dr != null)
                     {
                         ...
                         string _ProductArea_Id = string.IsNullOrEmpty((_dr.Cells["ProductArea_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["ProductArea_Id"].Value.ToString()).ToString();
                         try
                         {
                             bool _delOk = Delete(_Company_Id, _ProductArea_Id);
                             if (_delOk)
                             {
                                 string _where = " Where 1=1 and Company_Id='" + LoginInfo._Usr_Company + "'";
                                 QueryData(_where);
         ...
         {
             _ownerFrom.CloseThisFrom("ProductArea");
         }

         private void ProductAreaForm_Load(object sender, EventArgs e)
         {
