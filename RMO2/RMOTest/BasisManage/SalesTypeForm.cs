...
{
    public partial class SalesTypeForm : UserControl
    {
         private SubForm _ownerFrom;
         public SalesTypeForm(SubForm ownerForm)
        {
            InitializeComponent();
         ...
         private void QueryData(string _where)
         {
             ...
             strSql.Append("select ROW_NUMBER() over (order by getdate()) as  ITM,Company_Id,SalesType_Id,Desc_01,Parent_SalesType_Id,Remark,Usr__Id,Create__Date from SalesType ");
         ...
         /// 删除一条数据
         /// </summary>
         public bool Delete(string Company_Id, string SalesType_Id)
         {
             StringBuilder strSql = new StringBuilder();
             strSql.Append("delete from SalesType ");
             ...
             parameters[1].Value = SalesType_Id;
         ...
         private void BtnQuery_Click(object sender, EventArgs e)
         {
             ...
             {
                 _NotNullWhere.Append(string.Format("and SalesType_Id between '{0}'  and '{1}'", this.textBoxContainButton1.Text, this.textBoxContainButton2.Text));
             }
         ...
         {
             SalesTypeManage _Manage = new SalesTypeManage();
             _Manage.Edit = "ADD";
             ...
             if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
             {
                 if (dataGridView1.CurrentRow != null)
                 {
                     ...
                     if (_dr != null)
                     {
                         ...
                         _SalesType_Id = string.IsNullOrEmpty((_dr.Cells["SalesType_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["SalesType_Id"].Value.ToString()).ToString();
                     }
                     SalesTypeManage _Manage = new SalesTypeManage();
                     _Manage.Edit = "UPD";
                     _Manage.Company_Id = _Company_Id;
                     _Manage.SalesType_Id = _SalesType_Id;
                     if (_Manage.ShowDialog() != DialogResult.OK)
                     {
                         string _where = " Where 1=1 and Company_Id='" + LoginInfo._Usr_Company + "'";
                         QueryData(_where);
             ...
             if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
             {
                 if (dataGridView1.CurrentRow != null)
                 {
                     ...
                     if (_dr != null)
                     {
                         ...
                         string _SalesType_Id = string.IsNullOrEmpty((_dr.Cells["SalesType_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["SalesType_Id"].Value.ToString()).ToString();
                         try
                         {
                             bool _delOk = Delete(_Company_Id, _SalesType_Id);
                             if (_delOk)
                             {
                                 string _where = " Where 1=1 and Company_Id='" + LoginInfo._Usr_Company + "'";
                                 QueryData(_where);
         ...
         {
             _ownerFrom.CloseThisFrom("SalesType");
         }

         private void SalesTypeForm_Load(object sender, EventArgs e)
         {
         ...
         private void textBoxContainButton1_Click(object sender, EventArgs e)
         {
             ...
             {
                 string _where = "and SalesType.Company_Id='" + LoginInfo._Usr_Company + "'";
