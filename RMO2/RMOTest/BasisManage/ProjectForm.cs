    ...
    public partial class ProjectForm : UserControl
    {
         ...
         public ProjectForm(SubForm ownerForm)
        {
         ...
         private void QueryData(string _where)
         {
             ...
             strSql.Append("select ROW_NUMBER() over (order by getdate()) as  Itm,Company_Id,Project_Id,Desc_01,Parent_Company_Id,Parent_Project_Id,Remark,Usr__Id,Create__Date from Project ");
         ...
         public bool Delete(string Company_Id, string Project_Id)
         {
             StringBuilder strSql = new StringBuilder();
        ...
        private void BtnQuery_Click(object sender, EventArgs e)
        {
            ...
            {
                _NotNullWhere.Append(string.Format("and Project_Id between '{0}'  and '{1}'", this.textBoxContainButton1.ID, this.textBoxContainButton2.ID));
            }
        ...
        {
            ProjectManage _Manage = new ProjectManage();
            _Manage.Edit = "ADD";
            ...
            if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
            {
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    if (_dr != null)
                    {
                        ...
                        _Project_Id = string.IsNullOrEmpty((_dr.Cells["Project_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["Project_Id"].Value.ToString()).ToString();
                    }
                    ProjectManage _Manage = new ProjectManage();
                    _Manage.Edit = "UPD";
                    _Manage.Project_Id = _Project_Id;
            ...
            if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
            {
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    if (_dr != null)
                    {
                        ...
                        string _Project_Id = string.IsNullOrEmpty((_dr.Cells["Project_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["Department_Id"].Value.ToString()).ToString();
                        try
                        {
                            bool _delOk = Delete(_Company_Id, _Project_Id);
                            if (_delOk)
                            {
                                string _where = " Where 1=1 and Company_Id='" + LoginInfo._Usr_Company + "'";
                                QueryData(_where);
        ...
        {
            _ownerFrom.CloseThisFrom("Project");
        }
        ...
        private void textBoxContainButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                _ht = CommomHelper.GetQuery1("Project", _columns, "");
            }
            else
            {
                string _where = "and Project.Company_Id='" + LoginInfo._Usr_Company + "'";
