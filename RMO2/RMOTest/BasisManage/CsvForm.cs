...
{
    public partial class CsvForm : UserControl
    {
        private SubForm _ownerFrom;
        public CsvForm(SubForm ownerForm)
        {
            InitializeComponent();
        ...
        /// 删除一条数据
        /// </summary>
        public bool Delete(string Company_Id, string Csv_Id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Csv ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ROW_NUMBER() over (order by getdate()) as  ITM, Company_Id,Csv_Id,Desc_01,Parent_Csv_Id,Remark,Usr__Id,Create__Date from Csv ");
        ...
        {
            CsvManage _Manage = new CsvManage();
            _Manage.Edit = "ADD";
            ...
            if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
            {
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    if (_dr != null)
                    {
                        ...
                        _Csv_Id = string.IsNullOrEmpty((_dr.Cells["Csv_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["Csv_Id"].Value.ToString()).ToString();
                    }
                    CsvManage _Manage = new CsvManage();
                    _Manage.Edit = "UPD";
                    _Manage.Company_Id = _Company_Id;
                    _Manage.Csv_Id = _Csv_Id;
                    if (_Manage.ShowDialog() != DialogResult.OK)
                    {
                        string _where = " Where 1=1 and Company_Id='" + LoginInfo._Usr_Company + "'";
                        QueryData(_where);
            ...
            if (this.dataGridView1 != null && this.dataGridView1.Rows.Count > 0)
            {
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    if (_dr != null)
                    {
                        ...
                        string _Employee_Id = string.IsNullOrEmpty((_dr.Cells["Csv_Id"].Value.ToString()).ToString()) ? "" : (_dr.Cells["Csv_Id"].Value.ToString()).ToString();
                        try
                        {
